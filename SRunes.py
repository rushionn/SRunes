...
def update_actions_display(action):
    if action["type"] == "click":
        action_str = f"[click] X: {action['x']} Y: {action['y']} Button: {action['button']} Pressed: {action['pressed']}"
    elif action["type"] == "move":
        action_str = f"[move] X: {action['x']} Y: {action['y']}"
    elif action["type"] == "scroll":
        action_str = f"[scroll] X: {action['x']} Y: {action['y']} DX: {action['dx']} DY: {action['dy']}"
    elif action["type"] in ["keydown", "keyup"]:
        action_str = f"[{action['type']}] Key: {action['key']}"

    actions_display.insert("end", f"{action_str}\n")
            ...
            for action in loaded_actions:
                ...
                try:
                    ...
                    if action["type"] in ["click", "move"]:
                        action_str += f" X: {action.get('x', 'N/A')} Y: {action.get('y', 'N/A')}"
                    elif action["type"] in ["keydown", "keyup"]:
                        action_str += f" Key: {action['key']}"
                    elif action["type"] == "scroll":
                        action_str += f" DX: {action['dx']} DY: {action['dy']}"
                    actions_display.insert("end", f"{action_str}\n")
...
# 停止錄製按鈕
stop_button = ttk.Button(root, text="停止錄製", command=stop_recording)
