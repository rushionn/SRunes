...
def on_click(x, y, button, pressed):
    if recording and not paused:
        ...
        print(f"點擊事件: ({x}, {y}), 按鈕: {button_name}, 按下: {pressed}")
        action = {
            "type": "click",
            "x": x, "y": y,
            "button": button_name,
    ...
    if recording and not paused:
        try:
            ...
            if key_name in NUMPAD_MAPPING:
                key_name = NUMPAD_MAPPING[key_name]
...
def start_recording():
    ...
    if not recording:
        ...
        record_button.config(text="錄製中", style="success.TButton")
...
def stop_recording():
    ...
    if recording:
        recording = False
        save_actions()
        record_button.config(text="開始錄製", style="info.TButton")
...
def choose_file():
    ...
    file_path = filedialog.askopenfilename(filetypes=[("JSON檔案", "*.json")])
    if file_path:
        current_filename = file_path
        file_name_label.config(text=f"選擇的檔案: {Path(file_path).name}")

# ===================== UI 設定 =====================
root = ttk.Window(themename="superhero")
root.title("KeymouseGo - 操作錄製器")

record_button = ttk.Button(root, text="開始錄製", command=start_recording, style="success.TButton")
record_button.grid(row=0, column=0, padx=5, pady=5)

stop_button = ttk.Button(root, text="停止錄製(F9)", command=stop_recording, style="info.TButton")
stop_button.grid(row=0, column=2, padx=5, pady=5)

choose_button = ttk.Button(root, text="選擇存檔", command=choose_file, style="info.TButton")
...
execute_button = ttk.Button(root, text="執行操作", command=execute_actions, style="success.TButton")
execute_button.grid(row=1, column=2, padx=5, pady=5)

file_name_label = ttk.Label(root, text="尚未選擇檔案", anchor="w")
